...
## 이미지 · 영상 생성 모델 · Image & video generation

`모델 종류`를 **이미지 생성 / 영상 생성**으로 바꾸면 디퓨전 모델용 계산기로 전환됩니다. 물리가 LLM과 다르기 때문에 계산도 다릅니다.

| | 텍스트 LLM 디코딩 | 디퓨전 (이미지·영상) |
|---|---|---|
| 병목 | 메모리 **대역폭** | **연산(FLOPS)** |
| KV 캐시 | 컨텍스트에 비례해 증가 | **없음** — 매 스텝 전체 latent 토큰 재계산 |
| 배치 | 처리량이 거의 선형 증가 | **거의 무의미** (실측 6~10% 향상에 VRAM 1.5배) |
| 지연 결정 요인 | 활성 파라미터 ÷ 대역폭 | 스텝 × latent 토큰 × 모델 크기 ÷ 장비 FLOPS |

**속도 숫자에는 출처 배지가 붙습니다.** 자체 벤치마크(단일 B200)를 기준점으로 삼아 연산량비·장비 FLOPS비로 환산한 값은 `실측 기준`, 벤치마크가 없어 루프라인으로 추정한 값은 `추정치`로 표시합니다. 이 구분이 필요한 이유는 실측에서 모델별 연산 효율이 **13~64%로 5배 갈렸기** 때문입니다 — 파라미터 수만으로 속도를 추정하면 틀립니다(실제로 6B Z-Image가 20B Qwen-Image보다 느립니다). SDXL 같은 UNet 계열은 DiT용 수식이 10배 이상 부풀려서 **속도를 아예 계산하지 않고** VRAM만 냅니다.

**VRAM은 백본 / 텍스트 인코더 / VAE·활성값으로 쪼개 보여줍니다.** 텍스트 인코더가 상주 메모리의 3분의 1을 차지하는 경우가 흔한데(Qwen-Image 20B 백본 + 8.3B 인코더 = 57.7GB) 사이징에서 자주 누락되기 때문입니다. 예측값은 실측 대비 4% 이내입니다.

**양자화는 VRAM만 줄이고 속도는 그대로 둡니다.** 실측에서 fp8 단독은 오히려 0.83배로 느렸고, `torch.compile`과 함께 써야 1.72배가 나왔습니다 — 그래서 양자화를 속도 이득으로 모델링하지 않습니다.

각 모델에는 **HuggingFace 링크와 라이선스 원문 링크**가 함께 표시됩니다. 라이선스가 지역을 제한하는 모델(예: MiniMax-H3는 EU·영국·한국·미국이 Excluded Territory)은 경고와 원문 링크를 띄우되 **수치는 그대로 계산합니다** — 라이선스를 신청할지, 어느 지역에서 돌릴지 판단하려면 사이징이 필요하기 때문입니다.
...
런타임 경로가 특이한 모델은 그 사실을 1급 정보로 표시합니다 — 모델 카드에는 없는 사실이고, 음성 모델을 고를 때 가장 먼저 알아야 할 것입니다. Qwen3-TTS는 transformers 코어에 모델 타입이 없지만 **공식 `qwen-tts` 패키지와 vLLM-Omni** 두 경로가 열려 있고, CosyVoice3는 벤더 로컬 스택이 torch 세대 불일치로 깨지는 대신 **vLLM-Omni가 정확한 체크포인트로 서빙**하며, Nemotron ASR은 스트리밍 전용이라 오프라인 `transcribe()`가 아닌 **NeMo cache-aware 스트리밍 스크립트 또는 NIM 컨테이너**로 돌립니다.
...
노드별 배치 탭에는 **양자화 사다리**(BF16 · FP8 · INT8 · INT4 4단, 막대 = usable 대비, 초록 fit · 빨강 초과)가 붙습니다 — LLM과 같은 시각 문법이되, 여기서도 양자화는 **VRAM만 옮기고 속도 주장은 하지 않습니다**. ✓ 는 실존을 확인한 **공개 양자화 체크포인트**(BFL 공식 fp8, Nunchaku SVDQuant, GGUF, CT2-int8, MLX 등)가 있는 티어이고, 나머지 행은 산술 what-if로 정직하게 라벨합니다. 디퓨전의 INT4 행은 서빙 경로가 없어 표시만 하고 선택은 막습니다(음성은 GGUF·CT2·MLX가 실존하므로 선택 가능). 각 모델 패널에는 모델 카드·벤더 config에서 큐레이션한 **파라미터 튜닝 가이드**(steps·guidance·shift·`att_context_size` 등, 행마다 출처 링크)가 함께 표시됩니다.
