...
(function (root) {
  ...
  const DATA = {
    // --- data/models.json: ornith-1.5-397b ---
    "하이브리드 Gated DeltaNet(선형)/Gated Attention 4:1 교차 — 60층 중 풀어텐션(KV 보유) 15개 층만, 나머지 45층은 선형어텐션(RNN형 고정 상태) · MoE(512 전문가 중 top-10 라우팅 + 공유 1, 총 396.8B 중 활성 파라미터는 공식 미공개 — top-10/512 라우터 수식으로 역산한 추정치 약 17.8B) · 262,144 컨텍스트(최대 약 1,000,000 확장 가능, YaRN factor 4.0) · 네이티브 비전-언어 멀티모달(image-text-to-text) · MIT 라이선스 · 아키텍처 Qwen3_5MoeForConditionalGeneration — vLLM 0.27.0 네이티브 등록(Qwen3_5MoeForConditionalGeneration, qwen3_5 모듈) · SGLang v0.5.13+ 네이티브(qwen3_5.py EntryClass)":
      "Hybrid Gated DeltaNet (linear)/Gated Attention in a 4:1 interleave — only 15 of 60 layers run full attention and carry KV, the other 45 run linear attention with a constant recurrent state · MoE (top-10 of 512 experts + 1 shared; the officially-disclosed active-param count is not published — router math back-calculated from config gives ≈17.8B of 396.8B total) · 262,144 context (extensible to ≈ 1,000,000 via YaRN factor 4.0) · native vision-language multimodal (image-text-to-text) · MIT license · architecture Qwen3_5MoeForConditionalGeneration — natively registered in vLLM 0.27.0 (Qwen3_5MoeForConditionalGeneration, qwen3_5 module) · SGLang v0.5.13+ native (qwen3_5.py EntryClass)",
    "vLLM 0.27.0 네이티브 등록(Qwen3_5MoeForConditionalGeneration, qwen3_5 모듈) — 하이브리드 GDN 멀티모달 MoE":
      "Natively registered in vLLM 0.27.0 (Qwen3_5MoeForConditionalGeneration, qwen3_5 module) — hybrid GDN multimodal MoE",
    "하이브리드 Gated DeltaNet(선형)/Gated Attention 4:1 교차 — 40층 중 풀어텐션(KV 보유) 10개 층만, 나머지 30층은 선형어텐션(RNN형 고정 상태) · MoE(256 전문가 중 top-8 라우팅 + 공유 1, 활성 파라미터 공식 표기 약 3B) · 262,144 컨텍스트(YaRN으로 약 1,000,000 확장) · 네이티브 비전-언어 멀티모달(image-text-to-text) · MIT 라이선스 · 아키텍처 Qwen3_5MoeForConditionalGeneration — vLLM 0.27.0 네이티브 등록(qwen3_5 모듈) · SGLang v0.5.13+ 네이티브(qwen3_5.py EntryClass)":
      "Hybrid Gated DeltaNet (linear)/Gated Attention in a 4:1 interleave — only 10 of 40 layers run full attention and carry KV, the other 30 run linear attention with a constant recurrent state · MoE (top-8 of 256 experts + 1 shared; officially stated active parameters ≈3B) · 262,144 context (extensible to ≈ 1,000,000 via YaRN) · native vision-language multimodal (image-text-to-text) · MIT license · architecture Qwen3_5MoeForConditionalGeneration — natively registered in vLLM 0.27.0 (qwen3_5 module) · SGLang v0.5.13+ native (qwen3_5.py EntryClass)",
    "Qwen3_5MoeForConditionalGeneration — 엔진 레지스트리 미등록, 지원 확인/실측 필요":
      "Qwen3_5MoeForConditionalGeneration — not registered in the engine's architecture list, support unconfirmed/needs a real test",
    ...
    "vLLM 0.27.0 네이티브 등록(Qwen3_5MoeForCausalLM, qwen3_5 모듈) — 하이브리드 GDN MoE, dense(Qwen3_5ForCausalLM)와 동일 qwen3_5 모듈 커널":
      "Natively registered in vLLM 0.27.0 (Qwen3_5MoeForCausalLM, qwen3_5 module) — hybrid GDN MoE, same qwen3_5 module kernels as the dense variant (Qwen3_5ForCausalLM)",
    ...
    "SGLang v0.5.18 Day-0 네이티브 등록(muse_glimmer.py) — 공식 릴리스 노트 'Muse Glimmer' 등재 · --tool-call-parser muse":
      "SGLang v0.5.18 Day-0 native registration (muse_glimmer.py) — listed in the official release notes ('Muse Glimmer') · --tool-call-parser muse",
    "SGLang v0.5.13+ 네이티브(qwen3_5.py EntryClass) — Qwen3.5 계열 하이브리드 GDN 멀티모달 MoE, dense와 동일 경로라 최신 빌드 권장":
      "SGLang v0.5.13+ native (qwen3_5.py EntryClass) — Qwen3.5-family hybrid GDN multimodal MoE, same path as the dense variant; a recent build is recommended",
    "SGLang v0.5.17 신규 네이티브(qwen3_5_text.py EntryClass) — Qwen3.5 계열 텍스트 전용 하이브리드 GDN MoE":
      "SGLang v0.5.17 newly native (qwen3_5_text.py EntryClass) — Qwen3.5-family text-only hybrid GDN MoE",
    ...
    "MoE(256 experts top-8 + 공유 1, 750B 중 37B active) · GQA 8 KV heads×128(kv_dim=1024) · 78층(full-attention과 sliding-window 어텐션 1:3 반복 패턴) · 262,144 컨텍스트 · 툴콜링 지원 · enable_thinking 기본 on(멀티턴 preserve_thinking 지원) · 한국 독자 AI 파운데이션 모델 프로젝트 2호 · Apache 2.0 · vLLM 0.27.0 네이티브 등록(ExaoneMoeForCausalLM) · SGLang은 클래스명 ExaoneMoEForCausalLM이 HF명과 case 불일치라 네이티브 미등록(LG 자체 서빙 포크 제공)": "MoE (256 experts top-8 + 1 shared, 37B active of 750B) · GQA 8 KV heads×128 (kv_dim=1024) · 78 layers (full-attention and sliding-window attention repeating in a 1:3 pattern) · 262,144 context · tool-calling supported · enable_thinking on by default (preserve_thinking for multi-turn) · the second model under Korea's Independent AI Foundation Model Project · Apache 2.0 · natively registered in vLLM 0.27.0 (ExaoneMoeForCausalLM) · not natively registered in SGLang (its class ExaoneMoEForCausalLM case-mismatches the HF arch name; LG ships its own serving forks)",
    "vLLM 0.27.0 네이티브 등록(ExaoneMoeForCausalLM, exaone_moe 모듈) — vLLM 공식 지원 문서에 K-EXAONE-2.0-750B-A37B 대표 모델로 등재":
      "Natively registered in vLLM 0.27.0 (ExaoneMoeForCausalLM, exaone_moe module) — listed in vLLM's official support docs as the K-EXAONE-2.0-750B-A37B representative model",
    "SGLang 클래스명은 ExaoneMoEForCausalLM — HF 아키텍처명(ExaoneMoeForCausalLM)과 case 불일치로 네이티브 미매칭, transformers 폴백 경로 예상":
      "SGLang's class is ExaoneMoEForCausalLM — no native match due to case mismatch with the HF arch name (ExaoneMoeForCausalLM); expect the transformers fallback path",
    ...
    "하이브리드 Mamba-2/MoE/어텐션 — KV는 어텐션 7개 층만(52층: mamba 23 · MoE 22 · 어텐션 7) · MoE(128 전문가 top-6 + 공유 1, 공식 총 30B / 활성 3B) · 262,144 컨텍스트(카드 최대 1M 주장) · OpenMDW-1.1":
      "Hybrid Mamba-2/MoE/attention — KV carried only by the 7 attention layers (52 layers: 23 mamba · 22 MoE · 7 attention) · MoE (top-6 of 128 experts + 1 shared; officially 30B total / 3B active) · 262,144 context (card claims up to 1M) · OpenMDW-1.1 license",
