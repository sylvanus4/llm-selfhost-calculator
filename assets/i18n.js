...
(function (root) {
  ...
  const DATA = {
    ...
    "하이브리드 Gated DeltaNet(선형)/Gated Attention 4:1 교차 — 60층 중 풀어텐션(KV 보유) 15개 층만, 나머지 45층은 선형어텐션(RNN형 고정 상태) · MoE(512 전문가 중 top-10 라우팅 + 공유 1, 총 396.8B 중 활성 파라미터는 공식 미공개 — top-10/512 라우터 수식으로 역산한 추정치 약 17.8B) · 262,144 컨텍스트(최대 약 1,000,000 확장 가능, YaRN factor 4.0) · 네이티브 비전-언어 멀티모달(image-text-to-text) · MIT 라이선스 · 아키텍처 Qwen3_5MoeForConditionalGeneration — vLLM/SGLang 공식 모델 레지스트리 미등록(GitHub 이슈에서 'not supported for now' 확인), 실측 전 --model-impl transformers 폴백 가정":
      "Hybrid Gated DeltaNet (linear)/Gated Attention in a 4:1 interleave — only 15 of 60 layers run full attention and carry KV, the other 45 run linear attention with a constant recurrent state · MoE (top-10 of 512 experts + 1 shared; the officially-disclosed active-param count is not published — router math back-calculated from config gives ≈17.8B of 396.8B total) · 262,144 context (extensible to ≈ 1,000,000 via YaRN factor 4.0) · native vision-language multimodal (image-text-to-text) · MIT license · architecture Qwen3_5MoeForConditionalGeneration — not registered in vLLM/SGLang's official model registry as of this writing (confirmed via GitHub issues reporting 'not supported for now'), assume a --model-impl transformers fallback pending a real test",
